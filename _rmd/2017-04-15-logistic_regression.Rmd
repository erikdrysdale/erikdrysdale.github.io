...
mdl1 <- glm(chd~sbp+tobacco+ldl+famhist+obesity+alcohol+age,data=dat,family=binomial(link=logit))
mdl1.coef <- coefficients(summary(mdl1))
library(stargazer)
stargazer(mdl1.coef,type='html',digits=3,title = 'Table 1A: Coefficient estimates for SA Heart',
          summary=F,rownames=T,header=F)
# print(xtable::xtable(mdl1.coef,digits=3,caption='Table 1A: Coefficient estimates for SA Heart'),
#       include.rownames=T,include.colnames=T,comment=F,caption.placement='top',type='html')
```
<br>
...
For this data set, the LR results suggest that becoming one-year older leads to an increase of the odds of coronary heart disease by `r round(exp(bw.coef['age','Estimate']),3)`. To determine statistical significance, confidence intervals (CI) can be constructed by appealing to the fact that the maximum likelihood estimate is (asymptotically) normal. If the CI for the odds contains "1", then the result is insignificant.

```{r,echo=F}
ci.age <- round(exp(c(bw.coef['age',1] - qnorm(0.975)*bw.coef['age',2],bw.coef['age',1] +  qnorm(0.975)*bw.coef['age',2])),3)
```

$$
\begin{align*}
&(1-\alpha)\% \text{ CI for the change in odds} \\
\exp \Big\{ \hat\beta_{\text{age}} &\pm Z_{1-\alpha/2}\sqrt{\hat{\text{Var}}(\hat\beta_{\text{age}})} \Big\} = \{`r ci.age[1]`,`r ci.age[2]` \}
\end{align*}
$$
