...
For this data set, the LR results suggest that becoming one-year older leads to an increase of the odds of coronary heart disease by `r round(exp(bw.coef['age','Estimate']),3)`. To determine statistical significance, confidence intervals (CI) can be constructed easily too by appealing to the fact that the maximum likelihood estimate is (asymptotically) normal. If the CI contains "1", then the result is insignificant.
...
\exp \Big\{ \hat\beta_j &\pm Z_{1-\alpha/2}\sqrt{\hat{\text{Var}}(\hat\beta_j)} \Big\} = \{`r ci.age[1]`,`r ci.age[2]` \}
\end{align*}
$$
