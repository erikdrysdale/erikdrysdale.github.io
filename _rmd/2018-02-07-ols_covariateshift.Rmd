---
title: "Transfer learning in regression: a statistical perspective"
...
<!-- It's quite possible that many of the things I cover have been pointed out elsewhere. However, if I dig further into this area I will be sure to include more references next time.  -->

## Linear regression: the simple case
...
The only twist is that the distribution of $\bx_S$ and $\bx_T$ differ. Note that $\bx_S$ is mean zero, where the target has mean general zero: this is done to simplify the math later, and it reasonable since we can always de-mean any training data that is given.[[^2]][[^3]] We conceptualize this data as having some observations from the third quadrant (the source), but wanting to make predictions about data in the first quadrant (the target).



### The inference case?

It should be noted that if $N_S=N_T$ and $\sigma^2_S=\sigma^2_T$ then the statistical efficiency of the least squares $\hat\beta$ is identical between the source and target domains. In other words, even if the covariates are shifted, the variance of the estimator of the linear relationship ($\beta$) is identical. This is intuitive and is easy to show.
...
The expected value of a random variable with a non-central F distribution is making the unconditional expectation easy to calculate: $N_S(1+(\mu_x/\sigma_x)^2)/(N_S-2)$.
...
Why is there this discrepancy between the inference and regression case?


## Adjustment procedures

Given that least squares has a transfer cost in terms of prediction error when the source covariate distribution differs to the target one, can anything be done to ameliorate this cost? Intuitively, a weighting scheme seems appropriate where source distribution observations that are closer to the the taret get upweighted. Indeed this strategy is known as ... 
