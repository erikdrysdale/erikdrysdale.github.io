...
```{r lb_indicator,fig.height=5,fig.width=6,warning=F,message=F,echo=F,eval=T}
library(cowplot)
x <- seq(-3, 3, length.out = 100)
sigmoid <- function(x) { 1/(1+exp(-x)) }
lbfun <- function(x) { 1 + log(sigmoid(x))/log(2) }
y <- lbfun(x)
df <- data.frame(x,y)
ggplot(data=df) + 
  geom_point(aes(x,y),color='blue') + 
  geom_step(data=data.frame(x=c(-3,0,1e-5,3),y=c(0,0,1,1)), aes(x,y),inherit.aes = F) + 
  background_grid(major='xy',minor='none') + 
  labs(title='Lower bound on indictator function') + 
  geom_text(data=data.frame(x=0.75,y=-0.5,lab='y==1+frac(log(sigma(x)),log(2))'),aes(x,y,label=lab),color='blue',parse=T)
...
```{r cindex_bhat,fig.height=9,fig.width=8,echo=F,eval=T}
load('sgd_data.RData')
library(cowplot)
library(ggrepel)
lst.save$gg.coef.plot
```
...
```{r cindex_sims,fig.height=9,fig.width=8,echo=F,eval=T}
lst.save$gg.boxplot.pv
```
