...
dir <- "C:/Users/erikinwest/Documents/bioeconometrician/github/bioeconometrician.github.io/_rmd/"
# setwd(dir)
...
```{r optimalshrink,echo=F,fig.width=6.5,fig.height=5.5}
sr <- seq(0.7,1,0.01)
...
```
```{r gcv,echo=F,,fig.width=6.5,fig.height=5.5}
...
ggplot(B,aes(x=b1,y=b2,z=z)) + 
  ...
  geom_text_repel(data=data.frame(x=beta[1],y=beta[2]),aes(x=x,y=y,label='OLS'),
                  color='red',size=4,inherit.aes = F,nudge_y = 0.2) +
  labs(x=expression(beta[1]),y=expression(beta[2]),
       subtitle='') +
  geom_text(data=data.frame(x=0,y=0),inherit.aes = F,parse=T,
            aes(x=x,y=y,label="beta[1]^2 * '+' * beta[2]^2 * '=0.' * 5^2"),size=5) +
  geom_text(data=data.frame(x=0.5,y=-0.75),aes(x=x,y=y),inherit.aes=F,color='blue',size=3,
  ...
  geom_text_repel(data=data.frame(x=beta.ridge[1],y=beta.ridge[2]),aes(x=x,y=y,label='Ridge'),
                  color='blue',size=4,inherit.aes = F,nudge_y = 0.2)
gg.cont
```
