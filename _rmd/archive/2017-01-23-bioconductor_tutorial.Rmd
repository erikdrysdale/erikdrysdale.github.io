...
ll <- c('tidyverse','magrittr','forcats','stringr','cowplot')
        # 'broom','scales','reshape2','ggrepel','GEOquery','genefilter','hgu133plus2.db')
sapply(ll, function(xx) library(xx,character.only = T))

load('C:/Users/erikinwest/Documents/Courses/CISC_875/tutorial/rmd_data.RData')
attach(rmd.list)
```
...
gg.surv.share
```
```{r cisc_surv,echo=F,fig.width=2,fig.height=2}
...
gg.her2
```
```{r cisc_her,echo=F,fig.height=2.0,fig.width=2.5}
...
As we have more than 19K genes, we will get back more than 19K p-values. This introduces the problem of multiple hypothesis testing, and therefore requires us to modify how we interpret our p-values. Correction procedures, such as the [Bonferroni](https://en.wikipedia.org/wiki/Bonferroni_correction) method can be employed. With a 5% threshold, we will now require a p-value to be less than $0.05/19149$ for us to reject the null. We often do a log-transformation of the new theshold value $-\log_{10}(0.05/19149)$=`r round(-log10(0.05/19149),1)` for easier visualization.
...
```{r cisc_volcano,echo=F,fig.height=2.0,fig.width=2.5}
theme_set(theme_cowplot(font_size=6))
gg.volcano$layers[[1]]$geom$default_aes$size <- 0.5
...
gg.topfour
```
```{r cisc_dens,echo=F,fig.height=2.0,fig.width=2.5}
theme_set(theme_cowplot(font_size=6))
gg.topfour$layers[[1]]$geom$default_aes$size <- 0.5
gg.topfour
```
...
gg.mds
```
```{r cisc_mds,echo=F,fig.height=2.2,fig.width=2.2}
theme_set(theme_cowplot(font_size=5))
gg.mds
```
...
gg.manhatt
```
```{r cisc_manhattan,echo=F,fig.width=2.5,fig.height=2.0}
theme_set(theme_cowplot(font_size=5))
gg.manhatt <- gg.manhatt + theme(axis.text.x = element_text(size=4))
gg.manhatt$layers[[1]]$geom$default_aes$alpha <- 0.5
gg.manhatt$layers[[3]]$geom$default_aes$alpha <- 1
gg.manhatt$layers[[3]]$aes_params$size <- 1.0
gg.manhatt
```
