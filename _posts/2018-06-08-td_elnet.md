...
elnet.prox.cox <- function(So,X,lambda,alpha=1,standardize=T, tol=1e-5,maxstep=1e4, bb=NULL, lammax=F) {
  ...
  res <- resfun(X, bhat, Y, tY, lambda, alpha, Pfun, d, llfun) # Barzilai Borwein (1)
  grad <- gradfun(X, res$res, bhat, lambda, alpha)
  
  gtol <- max(abs(grad))
  if (alpha == 0) { gtol <- gtol / 1e-4 } else { gtol <- gtol / alpha }
  if (lammax) { return(gtol) } # Should return?
  if (lambda >= gtol) { return(rep(0,p)) } 
  
  bhat.new <- proxstep(bhat, grad, 0.01*gtol, alpha, lambda) # Barzilai Borwein (2)
  ...
  bhat.scale <- as.vector(bhat / sd.X) # re-scale
  
  err.KKT1 <- sum((grad.new[bhat != 0]-alpha*lambda)^2) # Check KKT conditions
...
So.ti <- with(veteran,Surv(time,status)) # Time-invariant data (veteran)
X.ti <- model.matrix(~factor(trt)+karno+diagtime+age+factor(prior), data=veteran)[,-1]
 
So.td <- with(heart,Surv(start, stop, event)) # Time-dependent data (heart)
X.td <- model.matrix(~age+year+surgery+transplant, data=heart)[,-1]
 
mdl1.coxph <- coxph(So.td ~ X.td,ties = 'breslow') # (i) Standard cox model
...
round( data.frame(glmnet=as.vector(coef(mdl2.glmnet)), bb=mdl2.wrapper$bhat ), 5)
{% endhighlight %}
...
round( data.frame(glmnet=as.vector(coef(mdl3.glmnet)), bb=mdl3.wrapper$bhat ), 5)
{% endhighlight %}
...
round( data.frame(glmnet=as.vector(coef(mdl4.glmnet)), bb=mdl4.wrapper$bhat ), 5)
{% endhighlight %}
...
Of course to point of the `elnet.prox.cox` is not to replicate `coxph` or `glmnet` but rather regularized time-dependent cox models. We will fit the elastic-net solution path for $\alpha = [1/3, 2/3, 1]$ using our model on the `cgd` dataset.
