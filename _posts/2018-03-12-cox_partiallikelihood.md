...
Where $\pi_{ij}$ represents person $i$'s relative risk score at time $j$, or alternatively their softmax score at time $t_j$. If the observations are ordered so that $t_1 < \dots < t_N$ then $\bP$ will be a lower triangular matrix where $[\bP]_{ij}=\pi_{ij}$. Finally, to recover $\frac{\partial \ell}{\partial \bbeta}$ notice that:
