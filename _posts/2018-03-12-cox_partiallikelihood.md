...
Lastly, we can combine both the Ridge and Lasso models into the single elastic-net framework: $P(\lambda,\alpha,\bbeta)=$ $\lambda(\alpha$ $\\| \bbeta \\|_{1}$ $+0.5(1-\alpha)$ $\\| \bbeta \\|_{2}^2 $ $)$ and once again use proximal gradient descent to update our model:
