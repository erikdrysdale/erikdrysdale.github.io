...
Lastly, we can combine both the Ridge and Lasso models into the single elastic-net framework: $P(\lambda,\alpha,\bbeta)=\lambda(\alpha$ words $\lambda \\| \bbeta \\|_1$ and once again use proximal gradient descent to update our model:
